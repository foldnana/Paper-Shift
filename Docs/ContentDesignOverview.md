...
4. **动作限定条件**：事件和条件效果可用 `ActionIs` 限定在 `PrepareInterview`、`AttendInterview`、`WorkProbation`、`ApplyRegularization` 等具体操作上。
5. **试用时间条件**：事件可用 `CurrentJobMonthsAtMost` / `CurrentJobMonthsAtLeast` 限定当前工作开始后的月份，避免“第一天”事件在后期出现。
6. **统一事件池**：普通消息、观看弹窗、选择弹窗都由 `Events` 提供，不再使用旧的辅助内容表。
7. **事件展示方式**：`notice=Log/Banner` 直接在主界面显示并结算效果；`notice=Modal` 或带 `option` 的事件弹出事件窗口。

准备面试、参加面试、试用期工作、申请转正都会抽取一个符合条件的事件。高频消息事件用较高 `baseWeight` 和 `cooldown=-1` 承担日常反馈；弹窗或选择事件用较低权重控制出现频率。强制事件仍会优先触发。
...
| 工作标签 | 12 | 8 | 20 |
| 公司 | 2 | 19 | 21 |
| 事件 | 21 | 41 | 58（部分同 ID 覆盖，含消息/弹窗/选择事件） |
| 后半生规则 | 13 | 17 | 30 |
